...
void createProcess(PCB_Queue_p createdQueue, int pid) {
	...
	if (num_comp_intensive_processes == MAX_COMPUTE_INTENSIVE_PROCESSES 
				&& num_io_processes == MAX_IO_PROCESSES) {
	...
	while (!valid_process) {
		...
		if (new_pcb->pid == 0){
			...
			if (process_type){
				if (num_io_processes < MAX_IO_PROCESSES) {
